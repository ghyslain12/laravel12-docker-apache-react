...
import {Sale, SalePayload} from '../types/sale.types.ts';
...
export default function SaleForm() {
    ...
    const mutation = useMutation({
        mutationFn: async (formData: SaleFormData) => {
            const selectedMaterialIds = Object.entries(formData.materials)
                .filter(([_, checked]) => checked === true)
                .map(([materialId, _]) => Number(materialId)); // On convertit l'ID en nombre

            const dataToSend: SalePayload = {
            ...
            if (id) {
                const response = await update<Sale, SalePayload>('/sale', Number(id), dataToSend);
                return response.data;
            } else {
                const response = await create<Sale, SalePayload>('/sale', dataToSend);
                return response.data;
            }
        },
        onSuccess: () => {
