...
it('clears sale cache when a sale is updated', function () {
    ...
    $response = $this->withToken($token)->putJson("/api/sale/{$sale->id}", $updateData);
...
it('returns fresh data from index after an update invalidates the cache', function () {
    ...
    $sale = Sale::factory()->create();
    $materials = \App\Models\Material::factory()->count(2)->create();
    $sale->materials()->attach($materials->pluck('id'));
    ...
    $this->withToken($token)->putJson("/api/sale/{$sale->id}", $updateData)->assertOk();
